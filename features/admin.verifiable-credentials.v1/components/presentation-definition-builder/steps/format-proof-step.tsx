...
const FormatProofStep: FunctionComponent<FormatProofStepProps> = ({
    ...
    const handleFormatChange = (index: number, format: VCFormat) => {
        const validAlgorithms = getValidAlgorithmsForFormat(format);
        const currentAlgorithm = formData.credentials[index].proofAlgorithm;

        // If current algorithm is not valid for new format, select first valid one
        const newAlgorithm = validAlgorithms.includes(currentAlgorithm)
            ? currentAlgorithm
            : validAlgorithms[0];

        updateCredential(index, {
            format,
            proofAlgorithm: newAlgorithm
        });
    };
