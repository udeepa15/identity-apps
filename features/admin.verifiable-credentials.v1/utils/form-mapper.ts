...
function mapInputDescriptorToCredential(descriptor: any): CredentialRequirement {
    ...
    if (descriptor.format) {
        // Handle SD-JWT string format
        if (typeof descriptor.format === 'string' && descriptor.format === VCFormat.SD_JWT_VC) {
            cred.format = VCFormat.SD_JWT_VC;
            // Default algo for SD-JWT if not specified (since string format doesn't have algo field)
            cred.proofAlgorithm = ProofAlgorithm.ES256;
        } else if (descriptor.format[VCFormat.SD_JWT_VC]) {
            // Handle SD-JWT object format (Prioritize SD-JWT in hybrid mode)
            cred.format = VCFormat.SD_JWT_VC;
            const sdJwtFormat = descriptor.format[VCFormat.SD_JWT_VC];
            if (sdJwtFormat["sd-jwt_alg_values"] && sdJwtFormat["sd-jwt_alg_values"].length > 0) {
                cred.proofAlgorithm = sdJwtFormat["sd-jwt_alg_values"][0] as ProofAlgorithm;
            } else if (sdJwtFormat.alg && sdJwtFormat.alg.length > 0) {
                // Fallback for legacy/alternative structure
                cred.proofAlgorithm = sdJwtFormat.alg[0] as ProofAlgorithm;
            }
    ...
    if (descriptor.constraints && Array.isArray(descriptor.constraints.fields)) {
        descriptor.constraints.fields.forEach((field: any) => {
            ...
            if (paths.some((p: string) => p.includes("credentialSubject")) || paths.includes("$.email")) {
                let fieldName = "";

                // Try to find a path that contains credentialSubject
                const subjectPath = paths.find((p: string) => p.includes("credentialSubject"));
                if (subjectPath) {
                    const fieldNameMatch = subjectPath.match(/credentialSubject\.([^"\s]+)/);
                    fieldName = fieldNameMatch ? fieldNameMatch[1] : "";
                } else if (paths.includes("$.email")) {
                    // Special case for email simple path
                    fieldName = "email";
                }
